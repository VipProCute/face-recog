...
static float* vmx_CopyTensorToFloat32Buffer
    (
    ...
    )
{
    ...
    // int num_features = sz / stride;
    // printf("[%s()]::[%d] num_features = %d\n", __FUNCTION__, __LINE__, num_features);
    ...
    )
{
    ...
    buffer = vmx_CopyTensorToFloat32Buffer(graph, tensor);
...
std::vector<float> vnn_PostProcessSfaceExtractFeature(vsi_nn_graph_t *graph)
{
    ...
    return std::vector<float>(tensor_data, tensor_data + sz);
