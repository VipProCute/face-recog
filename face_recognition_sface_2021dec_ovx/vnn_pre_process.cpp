...
static void _load_input_meta()
{
    ...
    input_meta_tab[0].image.scale[0] = 1.0;
    input_meta_tab[0].image.scale[1] = 1.0;
    input_meta_tab[0].image.scale[2] = 1.0;
...
#endif
    data = _get_jpeg_data(tensor, &meta, input_buf);
    ...
    )
{
    ...
    {
        status = _handle_multiple_inputs(graph, i, input_buf);
        TEST_CHECK_STATUS(status, final);
    }
